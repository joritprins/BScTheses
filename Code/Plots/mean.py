...
x_ = np.arange(0, 55, 0.1)
def plot(dir: str, file_names: str, runs: int):
    ...
    return (np.mean(interp_client, 0), np.mean(interp_server, 0))

cheetah_resnet50_client, cheetah_resnet50_server = plot('Code/Plots/cheetah_resnet50_50', 'cheetah_resnet50', 51)
cheetah_sqnet_client, cheetah_sqnet_server = plot('Code/Plots/cheetah_sqnet_50', 'cheetah_sqnet', 51)
SCI_HE_sqnet_client, SCI_HE_sqnet_server = plot('Code/Plots/SCI_HE_sqnet_11', 'SCI_HE_sqnet', 11)
# SCI_HE_resnet50_client, SCI_HE_resnet50_server = plot('Code/Plots/SCI_HE_resnet50_11', 'SCI_HE_resnet50', 11)
SCI_HE_resnet50_client, SCI_HE_resnet50_server = [], []
...
plt.xlabel("Time (s)")
plt.ylabel("Power consumption (Wh)")
plt.title("Power usage of client")
plt.plot(x_, cheetah_resnet50_client, label="Cheetah resnet50")
plt.plot(x_, cheetah_sqnet_client, label="Cheetah sqnet")
plt.plot(x_, SCI_HE_sqnet_client, label="SCI_HE resnet50")
# plt.plot(x_, SCI_HE_resnet50_client, label="SCI_HE sqnet")
plt.legend()
plt.savefig("Code/Plots/Means/{}_client.png".format(os.path.basename(__file__).partition(".py")[0]))
