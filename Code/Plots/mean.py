...
for i in range(0,runs):
    ...
    arr.append(np.array(json.loads(json_string)))
    if max < (arr[-1][-1]['host']['timestamp']-arr[-1][0]['host']['timestamp']):
        max = (arr[-1][-1]['host']['timestamp']-arr[-1][0]['host']['timestamp'])

x_ = np.arange(0, 65, 0.1)
results = []

for i in range(0, runs):
    start = arr[i][0]['host']['timestamp']
    filtered_client = filter_results(arr[i], PID_dict[str(i)]['client'], start=start)
    filtered_server = filter_results(arr[i], PID_dict[str(i)]['server'], start=start)
