"""
Plots graph for Power Consume measurements
"""
...
def plot(dir: str, file_names: str, runs: int, end: int, size=(int, int)):
    ...
    plt.ylim(0, max(round(np.max(interp_client)), round(np.max(interp_server)))*1.2)
    plt.xlim(0, end)
    plt.xlabel("Time (s)")
...
xcr, cheetah_resnet50_client, cheetah_resnet50_server = plot('Code/Plots/Results/same-device/cheetah-resnet50-50', 'cheetah-resnet50', runs=50, end=256, size=(10,4)) # 256
xsr, SCI_HE_resnet50_client, SCI_HE_resnet50_server = plot('Code/Plots/Results/same-device/SCI_HE-resnet50-50', 'SCI_HE-resnet50', runs=50, end=560, size=(10,4)) #560
